...
public class ControllerSala {
	...
	public String editarSala(Model model,@ModelAttribute SalaDto salaDto, @RequestParam String numero) {
		try {
			Sala sala = salaService.procurarSalaPorId(salaDto.getId());
			
			sala.setNumero(salaDto.getNumero());
			if(salaDto.getLocatarioId() != null ) {
				Locatario locatario = locatarioService.procurarLocatarioPorId(salaDto.getLocatarioId());
